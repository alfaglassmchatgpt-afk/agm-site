(() => {
  ...
  const ops = JSON.parse($('#operations-data').textContent), opById = new Map(ops.map(o => [o.id,o]));
  const key = 'agm-materials-request-v1', tintedPage=document.body.dataset.page==='tinted', mirrorPage = ['mirror','tinted'].includes(document.body.dataset.page);
  const baseNames = {standard:'обычное стекло',clear:'осветлённое стекло',tint_bronze:'бронза',tint_silver:'серебро',tint_gold:'золотое',tint_pink:'розовое',tint_blue:'синее',tint_dichroic:'дихроичное',tint_rainbow:'радужное'};
  const materialName = i => i.base.startsWith('tint_')?'Тонированное зеркало':'Зеркало серебро';
  const onThisPage = i => i && (i.base.startsWith('tint_')===tintedPage);
  let state = {items:[],active:null}, nextDraft = {base:tintedPage?'tint_bronze':'standard',thickness:'4'}, opDialogId=null, returnFocus=null, files=[];
  ...
  function normalize(i) {
    if (!i || !baseNames[i.base] || !['4','6'].includes(String(i.thickness))) return null;
    return {id:typeof i.id==='string'?i.id:newId(),base:i.base,thickness:String(i.thickness),width:String(i.width||'').slice(0,12),height:String(i.height||'').slice(0,12),quantity:String(i.quantity||'1').slice(0,5),unknown:!!i.unknown,ops:Array.isArray(i.ops)?[...new Set(i.ops.filter(x=>opById.has(x)))]:[],packaging:!!i.packaging,note:typeof i.note==='string'?i.note.slice(0,2000):'',bevel:{width:bevelWidths.includes(i.bevel?.width)?i.bevel.width:'',scope:i.bevel?.scope==='sides'?'sides':'all',sides:clean(i.bevel?.sides)},drill:{rows:Array.isArray(i.drill?.rows)&&i.drill.rows.length?i.drill.rows.slice(0,20).map(hole):[hole({})],location:clean(i.drill?.location,1000)}};
  }
  try { const saved=JSON.parse(localStorage.getItem(key)); if(saved && Array.isArray(saved.items)){state.items=saved.items.slice(0,30).map(normalize).filter(Boolean);state.active=state.items.some(i=>i.id===saved.active)?saved.active:state.items[0]?.id||null;} } catch(e) {}
  ...
  function itemLabel(i){return materialName(i)+' · '+i.thickness+' мм';}
  ...
  function renderList(){
    ...
    state.items.forEach((i,n)=>{const row=node('div',undefined,'cart-item'+(i.id===state.active?' active':''));
      const select=node('button',undefined,'select-item');select.type='button';select.dataset.selectItem=i.id;select.setAttribute('aria-pressed',String(i.id===state.active));select.append(node('strong',(n+1)+'. '+itemLabel(i)),node('span',baseNames[i.base]),node('span',dimensions(i)+' · '+i.quantity+' шт.'));
  ...
  function render(){
    ...
    if(mirrorPage){$$('input[name=base]').forEach(r=>r.checked=r.value===selection.base);$$('input[name=thickness]').forEach(r=>r.checked=r.value===selection.thickness);$('#variant-note').textContent=materialName(selection)+': '+baseNames[selection.base]+', '+selection.thickness+' мм.'+(i&&!onThisPage(i)?' В расчёте выбрано изделие другого материала. Нажмите «Добавить ещё зеркало», чтобы добавить этот вариант.':'');$('#add-material').textContent=i?'Добавить ещё зеркало →':'Добавить к расчёту →';}
  }
  ...
  function renderProcessing(){
    ...
    if(i.ops.includes('bevel')){
      ...
      box.append(node('p','Допустимость ширины для выбранного зеркала подтвердим при расчёте.','small muted'));processing.append(box);
    }
  ...
  function addItem(){
    if(state.items.length>=30){toast('В одной заявке можно собрать до 30 изделий.');return active();}
    const selection=mirrorPage?{base:$('input[name=base]:checked').value,thickness:$('input[name=thickness]:checked').value}:(active()||nextDraft);
    const item=normalize({id:newId(),base:selection.base,thickness:selection.thickness,quantity:'1',ops:[]});state.items.push(item);state.active=item.id;save();render();return item;
  }
  function ensureItem(){return active()||addItem();}
  function toggleOp(id,onlyAdd=false){
    if(!opById.has(id))return;const i=ensureItem();if(!i)return;
  ...
  $('#add-another').addEventListener('click',()=>{addItem();toast('Добавлено новое зеркало. У него свой набор обработок.');});
  if(mirrorPage){
    $('#add-material').addEventListener('click',()=>{addItem();toast('Зеркало добавлено в «Мой расчёт»');openCart();});
    ...
    $$('[data-toggle-op]').forEach(b=>b.addEventListener('click',()=>{toggleOp(b.dataset.toggleOp);toast('Выбор обновлён в «Мой расчёт»');}));
    $$('[data-info]').forEach(b=>b.addEventListener('click',()=>{opDialogId=b.dataset.info;const op=opById.get(opDialogId);$('#op-title').textContent=op.title;$('#op-description').textContent=op.description;$('#op-note').textContent=op.note;$('#op-diagram').replaceChildren(b.closest('.operation').querySelector('svg').cloneNode(true));$('#op-add').textContent=active()?.ops.includes(op.id)?'Уже в расчёте':'Добавить к расчёту';$('#operation-dialog').showModal();}));
  ...
  $('#op-add').addEventListener('click',()=>{toggleOp(opDialogId,true);$('#operation-dialog').close();toast('Обработка добавлена к выбранному зеркалу');});
  ...
  function requestText(){return state.items.map((i,n)=>`${n+1}. ${materialName(i)} — ${baseNames[i.base]}, ${i.thickness} мм\nРазмеры: ${dimensions(i)}\nКоличество: ${i.quantity} шт.\nОбработки: ${i.ops.map(id=>opById.get(id).title).join(', ')||'не выбраны'}${processingText(i)}\nУпаковка в картон: ${i.packaging?'да':'нет'}${i.note?'\nКомментарий: '+i.note:''}`).join('\n\n');}
  function validation(){for(let n=0;n<state.items.length;n++){const i=state.items[n];const pe=processingError(i);if(pe)return [i,`Изделие ${n+1}: ${pe}`];if(!Number.isInteger(Number(i.quantity))||Number(i.quantity)<1||Number(i.quantity)>9999)return [i,`Изделие ${n+1}: укажите количество от 1 до 9999.`];if(!i.unknown&&(!Number.isFinite(Number(i.width))||!Number.isFinite(Number(i.height))||Number(i.width)<=0||Number(i.height)<=0||Number(i.width)>10000||Number(i.height)>10000))return [i,`Изделие ${n+1}: укажите размеры в мм или отметьте «Размеры пока не знаю».`];}return null;}
  $('#prepare').addEventListener('click',()=>{const invalid=validation();if(invalid){state.active=invalid[0].id;render();$('#cart-error').textContent=invalid[1];$('#cart-error').hidden=false;$('#cart-error').scrollIntoView({block:'nearest'});return;}closeCart();$('#request-preview').textContent=requestText();$('#export-status').textContent='';$('#request-dialog').showModal();});

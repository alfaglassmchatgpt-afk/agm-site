(() => {
  ...
  function validation(){if(!state.items.length)return [null,"Добавьте изделие к расчёту."];for(let n=0;n<state.items.length;n++){const i=state.items[n];if(i.ops.some(id=>!materials[i.material].operations.includes(id)))return [i,'Обработка недоступна для выбранного материала.'];const pe=processingError(i);if(pe)return [i,`Изделие ${n+1}: ${pe}`];if(!Number.isInteger(Number(i.quantity))||Number(i.quantity)<1||Number(i.quantity)>9999)return [i,`Изделие ${n+1}: укажите количество от 1 до 9999.`];if(!i.unknown&&(!window.AGMOrderModel.wholeMillimetres(i.width)||!window.AGMOrderModel.wholeMillimetres(i.height)||Number(i.width)>10000||Number(i.height)>10000))return [i,`Изделие ${n+1}: укажите размеры целыми числами от 1 до 10000 мм или отметьте «Размеры пока не знаю».`];}return null;}

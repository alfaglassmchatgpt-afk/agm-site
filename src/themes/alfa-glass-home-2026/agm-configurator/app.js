(() => {
  ...
  ['width','height'].forEach(id=>{const input=$('#'+id);input.step='1';input.inputMode='numeric';});
  ...
  function renderOps(){
    ...
    AGMOrderModel.operations(target,(i||nextDraft).thickness).forEach(id=>{const op=opById.get(id),card=node('article',undefined,'operation');card.dataset.opCard=id;
    ...
    $('#operations-title').textContent=(target.policy==='craft'?'Параметры изготовления: ':'Обработка: ')+target.title;
    $('#packaging').closest('label').hidden=target.policy==='craft';
  ...
  function render(){
    ...
    const profile=materials[i?.material||current];
    $('#unknown').closest('label').hidden=profile.policy==='craft';
    $('#width').max=profile.maxSize?.width||10000;$('#height').max=profile.maxSize?.height||10000;
  ...
  function renderProcessing(){
    ...
    if(i.ops.includes('drill')&&materials[i.material].policy!=='craft'){
      ...
      i.drill.rows.forEach((r,n)=>{
        ...
        const [dl,d]=field('Диаметр, мм','hole-diameter-'+n,r.diameter,'number');d.min='1';d.step='1';d.disabled=r.unknown;d.inputMode='numeric';d.addEventListener('input',()=>{r.diameter=d.value;commit();});
  ...
  function toggleOp(id,onlyAdd=false){
    if(!opById.has(id))return;const i=ensureItem();if(!i||!AGMOrderModel.operations(materials[i.material],i.thickness).includes(id))return;
  ...
  if(mirrorPage){
    ...
    $('.operation-grid').addEventListener('click',e=>{
      ...
      if(b){opDialogId=b.dataset.info;const op=opById.get(opDialogId);$('#op-title').textContent=op.title;$('#op-description').textContent=op.description;$('#op-note').textContent=op.note;let guide=$('#op-guide');if(!guide){guide=node('a','Подробнее об обработке →');guide.id='op-guide';guide.style.display='block';guide.style.margin='16px 0';$('#op-note').after(guide);}guide.href=config.processingUrl+'#'+op.id;$('#op-diagram').replaceChildren(b.closest('.operation').querySelector('svg').cloneNode(true));$('#op-diagram').style.height=op.image?'auto':'';$('#op-diagram').style.flexDirection=op.image?'column':'';if(op.image){const photo=node('img');photo.src=config.operationAssetsUrl+op.image;photo.alt=op.imageAlt||op.title;photo.style.cssText='display:block;width:100%;max-width:440px;height:auto;max-height:260px;object-fit:contain;margin:auto';const caption=node('p','Визуализация','small muted');$('#op-diagram').replaceChildren(photo,caption);}if(op.images?.length&&window.agmOperationGallery){window.agmOperationGallery($('#op-diagram'),op.images,config.operationAssetsUrl);}$('#op-add').textContent=active()?.ops.includes(op.id)?'Уже в расчёте':'Добавить к расчёту';$('#operation-dialog').showModal();}
    });
  ...
  function requestText(){return state.items.map((i,n)=>`${n+1}. ${materialName(i)} — ${baseName(i)}, ${thicknessName(i)}\nРазмеры: ${dimensions(i)}\nКоличество: ${i.quantity} шт.\nОбработки: ${i.ops.map(id=>opById.get(id).title).join(', ')||'не выбраны'}${processingText(i)}\n${materials[i.material].policy==='craft'?'Состав фиксирован: '+baseName(i):'Упаковка в картон: '+(i.packaging?'да':'нет')}${i.note?'\nКомментарий: '+i.note:''}`).join('\n\n');}
  function validation(){if(!state.items.length)return [null,"Добавьте изделие к расчёту."];for(let n=0;n<state.items.length;n++){const i=state.items[n];if(i.ops.some(id=>!AGMOrderModel.operations(materials[i.material],i.thickness).includes(id)))return [i,'Обработка недоступна для выбранного материала.'];if(!AGMOrderModel.fitsFormat(i,materials[i.material]))return [i,`Изделие ${n+1}: укажите ширину до ${materials[i.material].maxSize.width} мм и высоту до ${materials[i.material].maxSize.height} мм целыми числами. Рифление направлено вдоль высоты.`];const pe=processingError(i);if(pe)return [i,`Изделие ${n+1}: ${pe}`];if(!Number.isInteger(Number(i.quantity))||Number(i.quantity)<1||Number(i.quantity)>9999)return [i,`Изделие ${n+1}: укажите количество от 1 до 9999.`];if(!i.unknown&&(!window.AGMOrderModel.wholeMillimetres(i.width)||!window.AGMOrderModel.wholeMillimetres(i.height)||Number(i.width)>10000||Number(i.height)>10000))return [i,`Изделие ${n+1}: укажите размеры целыми числами от 1 до 10000 мм или отметьте «Размеры пока не знаю».`];}return null;}

...
(() => {
  ...
  $('#segment-brief').addEventListener('submit', e => {
    ...
    const text = ["ALFAGLASS — Задание на изготовление деталей","Компания / контактное лицо: "+data.get('name'),"Изделия и детали: "+data.get('task'),"Количество / состав партии: "+data.get('extra'),"Тип заказа: "+data.get('kind'),"Город / район: "+data.get('city'),"Желаемые сроки: "+data.get('deadline'),"Нужна доставка: "+(data.has('delivery')?'да':'не указано'),'Изображения и чертежи приложите отдельно.','Файл подготовлен локально. Данные не отправлены.'].join('\n\n');
    const url = URL.createObjectURL(new Blob(['\uFEFF'+text],{type:'text/plain;charset=utf-8'}));
    const a = document.createElement('a');a.href=url;a.download='alfaglass-furniture-request.txt';document.body.append(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);

...
if (class_exists('Kirki')) {
	...
	new \Kirki\Section(
		'search_section',
		[
			'title'       => esc_html__('Поиск', 'alfa-glass'),
			'priority'    => 50,
		]
	);
	new \Kirki\Field\Text(
		[
			'settings'    => 'search__title',
			'label'       => esc_html__( 'Заголовок страницы поиска', 'alfa-glass' ),
	...
	new \Kirki\Field\Textarea(
		[
			'settings'    => 'popular_queris',
			'label'       => esc_html__( 'Часто ищут', 'alfa-glass' ),
			'section'     => 'search_section',
			'description'=> esc_html__( 'Перечислите популярные запросы через запятую', 'alfa-glass' ),
			'default'     => '',
		]
	);
}
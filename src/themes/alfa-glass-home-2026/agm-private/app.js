...
(() => {
  ...
  $('#segment-brief').addEventListener('submit', e => {
    e.preventDefault();
    const data = new FormData(e.currentTarget);
    const text = ["ALFAGLASS — Моя идея для дома","Ваше имя: "+data.get('name'),"Что вы хотите сделать: "+data.get('task'),"Примерные размеры: "+data.get('extra'),"Для какого помещения: "+data.get('kind'),"Город / район: "+data.get('city'),"Желаемые сроки: "+data.get('deadline'),'Замер: '+(data.has('measure')?'нужен':'не указан'),"Нужны доставка и монтаж: "+(data.has('delivery')?'да':'не указано'),'Изображения и чертежи приложите отдельно.','Файл подготовлен локально. Данные не отправлены.'].join('\n\n');
    const url = URL.createObjectURL(new Blob(['\uFEFF'+text],{type:'text/plain;charset=utf-8'}));
    const a = document.createElement('a');a.href=url;a.download='alfaglass-private-request.txt';document.body.append(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);

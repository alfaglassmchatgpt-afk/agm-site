/* Architect prototype: local brief export only, no network submissions. */
(() => {
  'use strict';
  ...
  $('#architect-brief').addEventListener('submit', e => {
    e.preventDefault();
    const data = new FormData(e.currentTarget);
    const text = ['ALFAGLASS — задание на комплектацию объекта','Компания / контакт: '+data.get('company'),'Объект и состав заказа: '+data.get('task'),'Количество / объём: '+data.get('quantity'),'Стадия проекта: '+data.get('phase'),'Город / район: '+data.get('city'),'Желаемый срок: '+data.get('deadline'),'Замер: '+(data.has('measure')?'нужен':'не указан'),'Доставка и монтаж: '+(data.has('install')?'нужны':'не указаны'),'Чертежи и визуализации приложите отдельно.','Этот файл подготовлен локально. Данные не отправлены.'].join('\n\n');
    const url = URL.createObjectURL(new Blob(['\uFEFF'+text],{type:'text/plain;charset=utf-8'}));
    const a = document.createElement('a');a.href=url;a.download='alfaglass-object-specification.txt';document.body.append(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);
    $('#brief-status').textContent='Файл подготовлен. Передайте его менеджеру вместе с чертежами.';
  });
})();

...
(() => {
  ...
  $('#designer-brief').addEventListener('submit', e => {
    e.preventDefault();
    const data = new FormData(e.currentTarget);
    const text = ['ALFAGLASS — описание проекта дизайнера','Студия / имя: '+data.get('studio'),'Объект и задача: '+data.get('task'),'Город / район: '+data.get('city'),'Желаемый срок: '+data.get('deadline'),'Замер: '+(data.has('measure')?'нужен':'не указан'),'Доставка и монтаж: '+(data.has('install')?'нужны':'не указаны'),'Чертежи и визуализации приложите отдельно.','Этот файл подготовлен локально. Данные не отправлены.'].join('\n\n');
    const url = URL.createObjectURL(new Blob(['\uFEFF'+text],{type:'text/plain;charset=utf-8'}));
    const a = document.createElement('a');a.href=url;a.download='alfaglass-design-project.txt';document.body.append(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);
